...
class CartControllerTest {
    ...
    @Test
    void removeItem_returns204NoContent() throws Exception {
        doNothing().when(cartService).removeItem(any(), any());

        mockMvc.perform(delete("/api/cart/items/TRK-001"))
                .andExpect(status().isNoContent());

        verify(cartService).removeItem(any(), eq("TRK-001"));
    }
}
